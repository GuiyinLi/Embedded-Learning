...
/* added by hcj for debug */
static int uda134x_init_codec(struct snd_soc_codec *codec)
{
	...
	*/
	l3_write1(&pd->l3, (0x3<<1)+1, 0xef, 1);
	l3_write1(&pd->l3, (0x1<<1)+0, 0x1f, 1);
	l3_write1(&pd->l3, (0x2<<1)+1, 0x80, 1);
	l3_write1(&pd->l3, (0x6<<1)+0, 0x0, 1);
	l3_write1(&pd->l3, (0x4<<1)+1, 0x10, 1);
	l3_write1(&pd->l3, (0x2b<<1)+0, 0x10, 1);
	l3_write1(&pd->l3, (0x9<<1)+0, 0x50, 1);
	l3_write1(&pd->l3, (0xd<<1)+0, 0x21, 1);
	l3_write1(&pd->l3, (0x7<<1)+0, 0x01, 1);

	l3_write1(&pd->l3, (0x34<<1)+1, 0x3f, 1);
	l3_write1(&pd->l3, (0x35<<1)+1, 0x3f, 1);
	l3_write1(&pd->l3, (0x36<<1)+1, 0x3f, 1);
	l3_write1(&pd->l3, (0x37<<1)+1, 0x3f, 1);
	return 0;
}
	...
	struct snd_soc_dai *dai)
{
	...
	/* added by hcj for dbg */
	uda134x_init_codec(codec);
